...
## 2026-08-19 — Erdős 142: literal common-marker h=4,5,6,7,8 wall
...
`0<=epsilon<=1/20000`. A second compact q=9 theorem proves the exact
midpoint-peelability/strict-potential capacity `C_9=30`; common-offset Fubini
sharpens the plane cap to `30/81` and closes `h=8` through
`258/(1022544+sqrt(1044801773568))`, including every `epsilon=1/n` with
`n>=7926`. The in-repo compact replay is noncertifying without the external
DRAT payloads; only the all-26-proof mode may emit `CONCLUSION_EXACT_C9_30`,
and callers must also require exit zero and terminal `PASS_NONMUTATION`.
Phase-specific markers, target 35, common-marker horizons `h>=9`,
carved/context-owned `A/B`, almost-everywhere coercivity, other languages,
integer transfer, and a new `r_3` bound remain open.
