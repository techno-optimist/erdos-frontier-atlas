...
`0<=epsilon<=1/20000`. A second compact q=9 theorem proves
`30<=C_9<=31`; common-offset Fubini sharpens the plane cap to `31/81` and
closes `h=8` through the exact first density-gap root, including every
`epsilon=1/n` with `n>=1022543`. Phase-specific markers, target 35, the exact
value of `C_9`, common-marker horizons `h>=9`, carved/context-owned `A/B`,
other languages, integer transfer, and a new `r_3` bound remain open.
