...
> [Frontier Board](../README.md#chronos-frontier-board), and any
> `certificates/*/lean/proof.json` manifests. Do not edit by hand —
...
> The C0 count above is **gap-map quantities only**. This repo separately holds **2 machine-checked formal proof(s)** of theorems and refutations (not `[L,U]` brackets) — see *Machine-checked formal proofs* below.

## Machine-checked formal proofs

Proofs in this repo verified by a proof assistant, discovered from
`certificates/*/lean/proof.json`. These are the strongest evidence tier we carry —
a theorem's truth reduced to the assistant's kernel and its named axioms — but they
prove **theorems/refutations**, not bracketed quantities, so they sit beside the gap
map rather than inside its C0 count.

| result | prover | `sorry`-free | axioms | ours? |
|---|---|:-:|---|---|
| [The Jacobian Conjecture is false at n = 3 over Q](../certificates/jacobian-conjecture/lean) | Lean 4 + mathlib | ✓ | standard 3 | verification (the counterexample map is Alpoge's; condition… |
| [Ringel's 9-element oriented matroid is not stretchable](../certificates/ringel-nonstretchability/lean) | Lean 4 + mathlib | ✓ | standard 3 | the result is ours; theorem classical (BLVSWZ; Carroll eq.… |

*Axioms* `standard 3` = `[propext, Classical.choice, Quot.sound]`, Lean's base — no
`sorry`, no `native_decide` (which would put the compiler in the trusted base). Each
proof's `README.md` states its honest boundary (e.g. which inputs rest on transcription).
