...
| [`certificates/fk-square`](certificates/fk-square) | WS4 fence #2: exact Furstenberg–Katznelson axis-aligned-square table `D(1..9)` + density-fence framing (fences degenerate at `N=9` — no pinned density crossed in range); honest 10-min/`N` cutoff (`N=10` open); values = A227133, no novelty claimed |
