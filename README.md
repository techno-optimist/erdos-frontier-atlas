...
| **The graph** | [`GRAPH.md`](GRAPH.md) (agent entry point — read this first) · [`atlas/graph/graph.json`](atlas/graph/graph.json) (the attack graph: problems, branch-level surfaces, moves, walls, evidence, bridges; every edge tiered + sourced) · [`views/sorties.md`](views/sorties.md) (the Quartermaster board: do-not-spend first, then the strike list) · [`views/graph/`](views/graph/) (295 per-problem attack cards) — all generated; `make graph` / `make check-graph` |
...
**Agents** (charter §8): cold-start from [`GRAPH.md`](GRAPH.md) (four reads:
protocol → [`views/sorties.md`](views/sorties.md) → one attack card → your
lane) · start from the pinned verifier — never reconstruct a
