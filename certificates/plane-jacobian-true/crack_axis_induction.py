...
def prove_axis_D_fullsolve(D: int) -> Tuple[bool, dict]:
    ...
    # NOTE: this forcing loop assigns forced[s]=0 in EVERY branch (a tautology);
    # it is NOT the load-bearing gate. The real obstruction is `single_break`
    # above (each exotic coeff individually breaks const-Jac). This loop only
    # records that each exotic monomial, taken alone, is individually obstructed.
    ...
    info["n_forced"] = n_f
    info["status"] = (
        "EXOTIC_SINGLE_PAIR_OBSTRUCTED" if n_f == len(exotic) else "PARTIAL"
    )
...
def main() -> int:
    ...
    receipt = {
        ...
        "exit_ok": ok,
        "theorem": (
            "Axis-leading Keller maps: D<=3 full sp.solve => solutions are "
            "elementary or tame shear (D=2). For D=4, each exotic monomial "
            "individually (and in verified pairs) obstructs Keller; the full "
            "D=4 solve is intractable in-budget — NOT a complete seal."
        ),
    }
    out = os.path.join(
