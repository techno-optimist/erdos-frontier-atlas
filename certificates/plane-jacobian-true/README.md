...
| X-drop Wronskian in \(k[y][x]\) | **SEALED** identity (\(N\le 5\), \(dy\le 2\); leading-pair forcing in zero-tower slice only) |
| Poisson `{R,K}=0` ⇔ pure-power binary \(R\) | **SEALED** lattice \(d\le 5\) + disc \(d=2\) |
| Axis form ⇒ elementary/tame | full solve \(D\le 3\) **SEALED**; \(D=4\) single/pair exotic obstruction only (not a complete seal); degfree x-isolation \(N\le 8\) |
| Geo deg 1 on tame locus | **SEALED** |
| **Full unbounded plane JC** | **OPEN** — residual (two links): (front) arbitrary Keller → pure-power axis NF for all \(d\); (back) \(\deg_x(f)\le 1\) with arbitrary \(\deg_x(g)\) → tame |
