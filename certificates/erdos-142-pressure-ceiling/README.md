...
trust path, four planted-failure controls that must fail):
...
| file | role |
|---|---|
| `verify.py` | the checker: C1, C2a, C2b + four planted-failure controls; exact arithmetic throughout |
| `constants.json` | committed artifact (checked, never regenerated by replay; no timing fields) |
