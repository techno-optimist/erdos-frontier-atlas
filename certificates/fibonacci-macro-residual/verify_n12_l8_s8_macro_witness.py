#!/usr/bin/env python3
"""Independent verifier: n=12 L=8 S=8 split-weight macro witness."""
...
WIT = json.loads((HERE / "N12_L8_S8_WITNESS.json").read_text(encoding="utf-8"))
OUT = HERE / "N12_L8_S8_MACRO_WITNESS_RESULT.json"
...
def main():
    ...
    status = (
        "PASS_N12_L8_S8_SPLIT_MACRO_WITNESS"
        if flow_ok and tags_ok and connected and parikh_ok
        else "FAIL"
    )
    result = {
        "schema": "lead.n12_l8_s8_macro_witness.v1",
        "status": status,
        "state_count": S,
        "return_length": L,
        "n": N,
        ...
        "claim_boundary": (
            "Exact n=12 common-anchor split-weight length-eight Parikh-balanced "
            "macro on a 8-state clock. Not Erdős 142."
        ),
    }
