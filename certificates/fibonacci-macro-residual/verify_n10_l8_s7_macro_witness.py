#!/usr/bin/env python3
"""Independent verifier: n=10 L=6 S=7 split-weight macro witness."""
...
WIT = json.loads((HERE / "N10_L8_S7_WITNESS.json").read_text(encoding="utf-8"))
OUT = HERE / "N10_L8_S7_MACRO_WITNESS_RESULT.json"
...
def main():
    ...
    status = (
        "PASS_N10_L8_S7_SPLIT_MACRO_WITNESS"
        if flow_ok and tags_ok and connected and parikh_ok
        else "FAIL"
    )
    result = {
        "schema": "lead.n10_l8_s7_macro_witness.v1",
        "status": status,
        "state_count": S,
        "return_length": L,
        "n": N,
