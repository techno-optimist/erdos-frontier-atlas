#!/usr/bin/env python3
"""
Independent exact verifier for the n=4 length-three split-weight macro witness.
...
OUT = HERE / "N4_L3_MACRO_WITNESS_RESULT.json"

DELTA = {
    (s, d): WIT["delta"][s][d] for s in range(3) for d in range(3)
}
...
def expand_routes(selected):
    """Expand mult routes into unit route list of steps."""
    routes = {}
...
def main():
    assert len(set(TAG_PORTS.values())) == 5
    ...
    for name, origin in ORIGINS.items():
        ...
        source = (cin, *states)
        edges.append((f"{name}_out", source, anchor, tag_column(digits, states), OUTWARD_WEIGHT[name]))
    ...
    status = (
        "PASS_N4_L3_SPLIT_MACRO_WITNESS"
        if flow_ok and tags_ok and connected and parikh_ok
        else "FAIL"
    )
    result = {
        "schema": "lead.n4_l3_macro_witness.v1",
        "status": status,
        "state_count": 3,
        "return_length": 3,
        "n": 4,
        ...
        "claim_boundary": (
            "Exact n=4 common-anchor split-weight length-three globally "
