...
**Observation (equal-crit family), NOT proved here.** If all critical points
coincide at \(c\) and \(p(\beta)=0\), the roots are the \(n\) points
\(c+(\beta-c)\omega^k\), which lie *on* the circle of radius \(|\beta-c|\) about
\(c\). The tempting next step — "so \(r>1\) puts a root outside the unit disk" —
**does not follow as written**: the hypothesis places those \(n\) points in the
disk, not the whole circle, and \(n\) points on a circle can sit inside the unit
disk while the circle itself is not contained in it. A write-up asserting this as
a theorem was withdrawn during review pending a correct argument; the family is
covered here only by the sampled scans in `results/`, which found no
counterexample but prove nothing.
