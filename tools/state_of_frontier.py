...
Inputs:
  ...
  certificates/*/lean/proof.json   machine-checked formal proofs living in this repo
                              (a separate ledger: theorems/refutations, not bracketed
                              quantities — surfaced beside the gap map, never inside its C0 count)
...
def discover_formal_proofs():
    """Machine-checked proofs living in this repo, from certificates/*/lean/proof.json.

    These are a SEPARATE ledger from the gap map: they are theorems and refutations, not
    bounded [L,U] quantities, so they are surfaced in their own section and are deliberately
    NOT folded into the gap map's C0 count (which is only about bracketed quantities). Reads a
    committed manifest per proof -- no Lean toolchain is invoked at generation time.
    """
    proofs = []
    certs = ROOT / "certificates"
    if not certs.is_dir():
        return proofs
    for manifest in sorted(certs.glob("*/lean/proof.json")):
        try:
            m = json.loads(manifest.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError):
            continue
        m["_dir"] = manifest.parent.relative_to(ROOT).as_posix()
        proofs.append(m)
    return proofs
...
def generate():
    ...
    formal_proofs = discover_formal_proofs()
    ...
    w("> [Frontier Board](../README.md#chronos-frontier-board), and any")
    w("> `certificates/*/lean/proof.json` manifests. Do not edit by hand —")
    ...
    w("")
    if formal_proofs:
        w(f"> The C0 count above is **gap-map quantities only**. This repo separately holds "
          f"**{len(formal_proofs)} machine-checked formal proof(s)** of theorems and refutations "
          f"(not `[L,U]` brackets) — see *Machine-checked formal proofs* below.")
        w("")

    if formal_proofs:
        w("## Machine-checked formal proofs")
        w("")
        w("Proofs in this repo verified by a proof assistant, discovered from")
        w("`certificates/*/lean/proof.json`. These are the strongest evidence tier we carry —")
        w("a theorem's truth reduced to the assistant's kernel and its named axioms — but they")
        w("prove **theorems/refutations**, not bracketed quantities, so they sit beside the gap")
        w("map rather than inside its C0 count.")
        w("")
        w("| result | prover | `sorry`-free | axioms | ours? |")
        w("|---|---|:-:|---|---|")
        for p in formal_proofs:
            ax = p.get("axioms", [])
            ax_cell = ("standard 3" if ax == STANDARD_AXIOMS
                       else ("**non-standard: " + ", ".join(ax) + "**" if ax else "?"))
            sf = "✓" if p.get("sorry_free") else "✗"
            title = md_escape(p.get("title", p.get("theorem", p["_dir"])), 74)
            link = f"[{title}]({'../' + p['_dir']})"
            ours = md_escape(p.get("ours", ""), 60)
            w(f"| {link} | {md_escape(p.get('prover',''),20)} | {sf} | {ax_cell} | {ours} |")
        w("")
        w("*Axioms* `standard 3` = `[propext, Classical.choice, Quot.sound]`, Lean's base — no")
        w("`sorry`, no `native_decide` (which would put the compiler in the trusted base). Each")
        w("proof's `README.md` states its honest boundary (e.g. which inputs rest on transcription).")
        w("")
