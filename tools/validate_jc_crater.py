...
STATUS_META = {
    "REFUTED_ALL_N_GE_3":
        ("#dc2626", "Refuted for all n ≥ 3", "alln", "✕"),
    "REFUTED_SOME_FINITE_DIM":
        ("#ea9010", "Refuted in some finite dimension", "somedim", "⊘"),
    "OPEN":
        ("#2563eb", "Open", "open", "○"),
    "TRUE_THEOREM":
        ("#16a34a", "Survives (proven theorem)", "theorem", "✓"),
    "REFUTED_INDEPENDENTLY_PRE_2026":
        ("#6b7280", "Refuted independently, pre-2026", "indep", "■"),
}
...
def render_map_block(g, nodes, status, flags):
    ...
    for s in present:
        hexc, label, _, glyph = STATUS_META[s]
        c = counts[s]
    ...
    L.append("|:-:|:--|--:|:--|")
    for s in present:
        _, label, _, glyph = STATUS_META[s]
        L.append(f"| {glyph} | **{label}** | {counts[s]} | {_MEANING[s]} |")
    L.append("")
    ...
    for nid in ordered:
        cls = "root" if nid == "jacobian_conjecture" else STATUS_META[status[nid]][2]
        L.append(f'  {nid}["{_short(nodes[nid]["name"])}"]:::{cls}')
    for e in g["edges"]:
        arrow = "-->" if e["dimension_semantics"] == "dimension_preserving" else "-.->"
        L.append(f"  {e['from']} {arrow} {e['to']}")
    L.append("  classDef root fill:#7f1d1d,stroke:#000,color:#fff,stroke-width:3px;")
    for s in _STATUS_ORDER:
        hexc, _, cls, _ = STATUS_META[s]
        stroke = {"alln": "#7f1d1d", "somedim": "#92400e", "open": "#1e3a8a",
                  "theorem": "#14532d", "indep": "#374151"}[cls]
        L.append(f"  classDef {cls} fill:{hexc},stroke:{stroke},color:#fff;")
    L.append("```")
    L.append("")
    L.append("**Reading the map.** Colour = computed status (legend above). An "
             "arrow `X → Jacobian Conjecture` means *X implies JC*, so JC's fall "
             "pulls X down with it. **Solid** arrows are dimension-preserving "
             "(the full `for all n ≥ 3` refutation flows); **dashed** arrows are "
             "dimension-mixing reductions (only the weaker `in some finite "
             "dimension` flows). The lone blue node with no outward arrow — the "
             "**plane Jacobian Conjecture (n = 2)** — is the entire surviving "
